...
class PatchAttacker:
    def __init__(self, model, mean, std, device, image_size=244,epsilon=1,steps=500,step_size=0.05,patch_size=31,random_start=True):

        mean, std = torch.tensor(mean),torch.tensor(std)
        self.epsilon = epsilon / std
        self.epsilon_cuda=self.epsilon[None, :, None, None].to(device)
        self.steps = steps
        self.step_size = step_size / std
        self.step_size=self.step_size[None, :, None, None].to(device)
        ...
        self.lb = (-mean / std)
        self.lb=self.lb[None, :, None, None].to(device)
        self.ub = (1 - mean) / std
        self.ub=self.ub[None, :, None, None].to(device)
    ...
    def perturb(self, inputs, labels, loc=None,random_count=1):
        ...
        for _ in range(random_count):
            ...
            if self.random_start:
                init_delta = np.random.uniform(-self.epsilon, self.epsilon, [inputs.shape[0]*inputs.shape[2]*inputs.shape[3], inputs.shape[1]])
                ...
                x = inputs + torch.where(mask, torch.Tensor(init_delta).to(self.device), torch.tensor(0.).to(self.device))
